...
<div class="card">
    ...
    <div class="card-body card-block">
      <form action="{{ route('product.update', $item->id) }}" method="POST">
        ...
        <div class="form-group">
          <label for="prince" class="form-control-label">Harga Barang</label>
          <input  type="number"
                  name="prince" 
                  value="{{ old('prince') ? old('prince') : $item->prince }}" 
                  class="form-control @error('prince') is-invalid @enderror"/>
          @error('prince') <div class="text-muted">{{ $message }}</div> @enderror
        </div>
        <div class="form-group">
